...
<template>
  <div
    v-if="toothId && toothData"
    class="bg-white/95 backdrop-blur-xl rounded-l-[40px] border-l border-white/50 shadow-[-20px_0_50px_-15px_rgba(0,0,0,0.15)] overflow-hidden flex flex-col h-full ring-1 ring-black/[0.02]"
  >
    <!-- Header -->
    <div
      class="p-5 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10"
    >
      <div>
        <div class="flex items-center gap-2.5">
          ...
          <span
            v-if="toothData.cut"
            class="px-1.5 py-0.5 bg-red-50 text-red-500 rounded border border-red-100 text-[8px] font-black uppercase tracking-wider"
            >Extracted</span
          >
          <span
            v-if="toothData.implant"
            class="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded border border-slate-200 text-[8px] font-black uppercase tracking-wider"
            >Implant</span
          >
        </div>
        <p
      ...
      <button
        @click="emit('close')"
        class="p-1.5 hover:bg-slate-50 rounded-full transition-all text-slate-300 hover:text-slate-600 border border-transparent hover:border-slate-100"
      >
        <X class="w-4 h-4" />
      </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide">
      <section
        :class="{ 'opacity-40 grayscale pointer-events-none': toothData.cut }"
      >
        <h3
          class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-3"
        >
          PD
        </h3>
        <div class="grid grid-cols-2 gap-3">
          <!-- Buccal Card -->
          <div
            class="bg-white border border-slate-100 rounded-xl p-3 shadow-sm text-center group hover:border-[#0052ff]/20 transition-all"
          >
            <div
              class="text-xl font-black mb-0.5 flex items-center justify-center gap-0.5"
            >
              <span
                v-for="(val, i) in analysisData?.buccalPD"
                :key="i"
          ...
          >
            ...
            >
              <span
                v-for="(val, i) in analysisData?.palatalPD"
                :key="i"
                :class="parseInt(val) > 4 ? 'text-red-500' : 'text-[#0052ff]'"
              >
                {{ val }}{{ Number(i) < 2 ? "-" : "" }}
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          >
            ...
            >
              <span
                v-for="(val, i) in analysisData?.buccalCAL"
                :key="i"
                :class="parseInt(val) > 4 ? 'text-red-500' : 'text-[#0052ff]'"
              >
                {{ val }}{{ Number(i) < 2 ? "-" : "" }}
          ...
          >
            ...
            >
              <span
                v-for="(val, i) in analysisData?.palatalCAL"
                :key="i"
                :class="parseInt(val) > 4 ? 'text-red-500' : 'text-[#0052ff]'"
              >
                {{ val }}{{ Number(i) < 2 ? "-" : "" }}
      ...
      >
        <!-- BoP Diagram -->
        <div
          class="relative bg-slate-50/50 border border-slate-100 rounded-2xl p-5 flex flex-col items-center group/bop"
        >
          <!-- Percentage Badge -->
          <div
          ...
          <div class="relative w-20 h-20 mb-2">
            <svg
              viewBox="0 0 100 100"
              class="w-full h-full transform rotate-30"
            >
              <!-- Outer Hexagon Frame -->
              <polygon
              ...
              <path
                v-if="toothData.buccal.bop[0]"
                d="M50 50 L6.7 25 L50 0 Z"
                fill="#ef4444"
              />
              <!-- Top-Mid (Mid) -->
              <path
                v-if="toothData.buccal.bop[1]"
                d="M50 50 L50 0 L93.3 25 Z"
                fill="#ef4444"
              />
              <!-- Top-Right (Mesial/Distal) -->
              <path
                v-if="toothData.buccal.bop[2]"
              ...
              <!-- Bottom-Left -->
              <path
                v-if="toothData.lingual.bop[0]"
                d="M50 50 L6.7 25 L6.7 75 Z"
                fill="#ef4444"
              />
              <!-- Bottom-Mid -->
              <path
                v-if="toothData.lingual.bop[1]"
                d="M50 50 L6.7 75 L50 100 Z"
                fill="#ef4444"
              />
              <!-- Bottom-Right -->
              <path
                v-if="toothData.lingual.bop[2]"
                d="M50 50 L50 100 L93.3 75 Z"
                fill="#ef4444"
              />
        ...
        <!-- Plaque Diagram -->
        <div
          class="relative bg-slate-50/50 border border-slate-100 rounded-2xl p-5 flex flex-col items-center group/pi"
        >
          <!-- Percentage Badge -->
          <div
          ...
          <div class="relative w-20 h-20 mb-2">
            ...
            >
              ...
              <!-- Buccal Sites (Top) -->
              <path
                v-if="toothData.buccal.pi[0]"
                d="M50 50 L6.7 25 L50 0 Z"
                fill="#3b82f6"
              />
              <path
                v-if="toothData.buccal.pi[1]"
                d="M50 50 L50 0 L93.3 25 Z"
                fill="#3b82f6"
              />
              <path
                v-if="toothData.buccal.pi[2]"
                d="M50 50 L93.3 25 L93.3 75 Z"
                fill="#3b82f6"
              />
              ...
              <!-- Lingual Sites (Bottom) -->
              <path
                v-if="toothData.lingual.pi[0]"
                d="M50 50 L6.7 25 L6.7 75 Z"
                fill="#3b82f6"
              />
              <path
                v-if="toothData.lingual.pi[1]"
                d="M50 50 L6.7 75 L50 100 Z"
                fill="#3b82f6"
              />
              <path
                v-if="toothData.lingual.pi[2]"
                d="M50 50 L50 100 L93.3 75 Z"
                fill="#3b82f6"
              />
      ...
      <section
        ...
        :class="{ 'opacity-40 grayscale pointer-events-none': toothData.cut }"
      >
        <h3 class="text-[11px] font-black text-slate-800 mb-4">
          Analysis Summary
        </h3>
        <div class="space-y-3.5">
          ...
          <div
            ...
            class="flex justify-between gap-3 cursor-pointer group"
          >
            <div class="flex items-center gap-1">
              <span
                class="text-[10px] font-bold text-slate-400 border-b border-dotted border-slate-300 group-hover:text-[#0052ff] group-hover:border-[#0052ff] transition-all"
                >Prognosis K&C</span
              >
              <svg
                class="text-slate-200 group-hover:text-[#0052ff] transition-colors"
            ...
            <span
              ...
              >{{ analysisData?.prognosisKC || "N/A" }}</span
            >
          </div>
          ...
          >
            ...
            <span
              ...
              >{{ analysisData?.prognosisMN || "N/A" }}</span
            >
          </div>
          <div class="flex justify-between items-center pt-1">
            <span class="text-[10px] font-bold text-slate-400"
              >Buccal-Keratinized</span
            >
            <span class="text-[10px] font-black text-slate-700"
              >{{ analysisData?.buccalKTW }} mm</span
            >
          </div>
          <div class="flex justify-between items-center">
            <span class="text-[10px] font-bold text-slate-400"
              >Palatal-Keratinized</span
            >
            <span class="text-[10px] font-black text-slate-700"
              >{{ analysisData?.palatalKTW }} mm</span
            >
          </div>
          <div class="flex justify-between items-center">
          ...
          >
            ...
            <span class="text-[10px] font-black text-slate-700">{{
              getFurLabel(analysisData?.furcation)
            }}</span>
          </div>
        </div>
      </section>
      ...
      <!-- Note / Remark -->
      <section
        v-if="toothData.note && !isEditingNote"
        class="bg-yellow-50/50 border border-yellow-100 rounded-3xl p-6 shadow-sm group relative"
      >
        <div class="flex justify-between items-start mb-3">
          <h3
            class="text-[11px] font-black text-yellow-600 uppercase tracking-[0.15em]"
          >
            Note
          </h3>
          <button
            @click="startEditing"
            class="opacity-0 group-hover:opacity-100 p-1 hover:bg-yellow-100 rounded text-yellow-600 transition-all"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="12"
        ...
        <p
          class="text-xs font-bold text-slate-600 leading-relaxed whitespace-pre-wrap"
        >
          {{ toothData.note }}
        </p>
      </section>
      ...
      <section
        v-if="isEditingNote"
        class="bg-white border border-[#0052ff]/20 rounded-3xl p-6 shadow-xl ring-4 ring-[#0052ff]/5"
      >
        <h3
          class="text-[11px] font-black text-[#0052ff] uppercase tracking-[0.15em] mb-4"
        >
          Clinical Note
        </h3>
        <textarea
  ...
  <Teleport to="body">
    ...
    >
      <div
        v-if="prognosisModalType"
        class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm"
        @click="prognosisModalType = null"
      >
        <div
        ...
        >
          <div
            class="sticky top-0 bg-white/80 backdrop-blur-md px-8 py-6 border-b border-slate-50 flex items-center justify-between z-10"
          >
            <h2 class="text-lg font-black text-slate-800 tracking-tight">
              {{
                prognosisModalType === "MN"
                  ? "McGuire and Nunn (M&N)"
                  : "Kwok and Caton (K&C)"
              }}
            </h2>
            <button
              @click="prognosisModalType = null"
              class="w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:text-slate-600 transition-all"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
          ...
          <div class="p-8 space-y-10">
            ...
            <div v-if="prognosisModalType === 'MN'">
              <div class="flex items-center gap-3 mb-5">
                <span
                  class="bg-red-500 text-white text-[10px] font-black px-2 py-1 rounded-md uppercase tracking-wider"
                  >Classification</span
                >
                <h3
              ...
              <div
                class="overflow-hidden border border-slate-100 rounded-2xl shadow-sm"
              >
                <table class="w-full text-left border-collapse">
                  <thead>
                    <tr class="bg-slate-50">
                      <th
                        class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 w-32"
                      >
                        Prognosis
                      </th>
                      <th
                        class="px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100"
                      >
                        Criteria
                      </th>
                    </tr>
                  ...
                  <tbody class="text-[11px] font-medium text-slate-600">
                    ...
                    >
                      <td
                        class="px-4 py-4 font-black text-green-600 bg-green-50/30"
                      >
                        Good
                      </td>
                      <td class="px-4 py-4 leading-relaxed italic">
                        Control of etiologic factors and enough support to
                        enable the tooth to be maintained by the patient and
                        clinician.
                      </td>
                    </tr>
                    <tr
                    ...
                    >
                      <td
                        class="px-4 py-4 font-black text-blue-600 bg-blue-50/30"
                      >
                        Fair
                      </td>
                      <td class="px-4 py-4 leading-relaxed italic">
                        ~25% attachment loss, Class I furcation. Adequate
                    ...
                    >
                      <td
                        class="px-4 py-4 font-black text-yellow-600 bg-yellow-50/30"
                      >
                        Poor
                      </td>
                      <td class="px-4 py-4 leading-relaxed italic">
                        50% attachment loss, Class II furcation. Maintenance
                        difficult.
                      </td>
                    </tr>
                    <tr
                    ...
                    >
                      <td
                        class="px-4 py-4 font-black text-orange-600 bg-orange-50/30"
                      >
                        Questionable
                      </td>
                      <td class="px-4 py-4 leading-relaxed italic">
                        > 50% attachment loss, Class II/III furcation, Class II
                        mobility, poor crown/root ratio.
                      </td>
                    </tr>
                    <tr class="hover:bg-slate-50/50 transition-colors">
                      <td
                        class="px-4 py-4 font-black text-red-600 bg-red-50/30"
                      >
                        Hopeless
                      </td>
                      <td class="px-4 py-4 leading-relaxed italic">
                ...
                <table class="w-full text-left border-collapse">
                  ...
                  <tbody class="text-[11px] font-medium text-slate-600">
                    ...
                    >
                      ...
                      <td class="px-4 py-4 leading-relaxed italic">
                        Can be stabilized with treatment/maintenance; less
                        chance of breakdown.
                      </td>
                    </tr>
                    <tr
                    ...
                    >
                      ...
                      <td class="px-4 py-4 leading-relaxed italic">
                        Influenced by local/systemic factors that may or may not
                        be controlled.
                      </td>
                    </tr>
                    <tr
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                      <td
                        class="px-4 py-4 font-black text-black bg-slate-50/30"
                      >
                        Hopeless
                      </td>
                      <td class="px-4 py-4 leading-relaxed italic">
          ...
          <div
            class="p-8 bg-slate-50/50 border-t border-slate-50 text-center rounded-b-[32px]"
          >
            <p
              class="text-[10px] text-slate-400 font-bold uppercase tracking-widest italic"
            >
              Reference: Clinical Periodontology Standards
            </p>
          </div>
        </div>
      </div>
