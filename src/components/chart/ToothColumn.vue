...
<template>
  ...
  >
    ...
    <template v-if="order === 'standard'">
      ...
      <div class="h-6 border-b border-slate-400"><input v-model="toothData.mo" type="text" :disabled="toothData.extracted || toothData.implant" :placeholder="toothData.implant ? '0' : ''" class="w-full h-full text-center text-[11px] outline-none disabled:bg-slate-100/50" /></div>
      <div class="h-6 border-b border-slate-400"><input v-model="toothData.ktw" type="text" :disabled="toothData.extracted" class="w-full h-full text-center text-[11px] outline-none disabled:bg-slate-100/50" /></div>
      ...
      <div class="flex h-6 border-b border-slate-400" :class="{ 'pointer-events-none opacity-50': toothData.extracted }">
        <div v-for="site in SITE_INDEXES" :key="site" class="flex-1 border-r border-slate-200 last:border-r-0 cursor-pointer transition-colors" :class="toothData[surface].bop[site] ? 'bg-red-500' : ''" @click.stop="emit('toggleBop', id, surface, site)"></div>
      </div>
      <div class="flex h-6 border-b border-slate-400" :class="{ 'pointer-events-none opacity-50': toothData.extracted }">
        <div v-for="site in SITE_INDEXES" :key="site" class="flex-1 border-r border-slate-200 last:border-r-0 cursor-pointer transition-colors" :class="toothData[surface].pi[site] ? 'bg-blue-500' : ''" @click.stop="emit('togglePi', id, surface, site)"></div>
      </div>
      <div class="flex h-6 border-b border-slate-400">
        <input v-for="site in SITE_INDEXES" :key="site" :value="toothData[surface].rec[site]" type="text" :disabled="toothData.extracted" class="w-0 flex-1 text-center text-[10px] border-r border-slate-200 last:border-r-0 outline-none disabled:bg-slate-100/50" @input="emit('updateRec', id, surface, site, inputValue($event))" />
      </div>
      <div class="flex h-6 border-b border-slate-400">
        <input v-for="site in SITE_INDEXES" :key="site" :value="toothData[surface].pd[site]" type="text" :disabled="toothData.extracted" class="w-0 flex-1 text-center text-[10px] border-r border-slate-200 last:border-r-0 outline-none disabled:bg-slate-100/50" @input="emit('updatePd', id, surface, site, inputValue($event))" />
      </div>
      <div class="flex h-6 border-b border-slate-400 bg-slate-50/30">
        <input v-for="site in SITE_INDEXES" :key="site" type="text" :value="toothData[surface].cal[site] || '0'" class="w-0 flex-1 text-center text-[10px] font-bold text-slate-700 border-r border-slate-200 last:border-r-0 bg-transparent outline-none pointer-events-none" />
      </div>
    </template>

    <template v-else>
      <div class="flex h-6 border-b border-slate-400 bg-slate-50/30">
        <input v-for="site in SITE_INDEXES" :key="site" type="text" :value="toothData[surface].cal[site] || '0'" class="w-0 flex-1 text-center text-[10px] font-bold text-slate-700 border-r border-slate-400 last:border-r-0 bg-transparent outline-none pointer-events-none" />
      </div>
      <div class="flex h-6 border-b border-slate-400">
        <input v-for="site in SITE_INDEXES" :key="site" :value="toothData[surface].pd[site]" type="text" :disabled="toothData.extracted" class="w-0 flex-1 text-center text-[10px] border-r border-slate-200 last:border-r-0 outline-none disabled:bg-slate-100/50" @input="emit('updatePd', id, surface, site, inputValue($event))" />
      </div>
