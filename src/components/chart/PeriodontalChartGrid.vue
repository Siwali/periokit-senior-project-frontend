...
<template>
  <section class="mt-6 w-full bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
      ...
      <div class="w-fit mx-auto">
        ...
        <div class="flex items-end mb-1">
          <div class="flex flex-col bg-white border-l border-t border-slate-300 text-[9px] font-bold text-slate-500 uppercase w-20 sticky left-0 z-20">
            ...
            <div v-for="row in BUCCAL_ROWS" :key="row" class="h-6 flex items-center px-2 border-b border-r border-slate-300">{{ row }}</div>
          </div>
          <div class="flex">
            ...
            <template v-for="(group, gIdx) in UPPER_ARCH" :key="gIdx">
              <div class="flex">
                <div v-for="id in group" :key="id" class="w-12 sm:w-[54px] h-8 flex items-center justify-center">
                  <button class="w-full h-6 text-[9px] font-black transition-all duration-200 border border-slate-200" :class="chartData[id].extracted ? 'bg-red-500 text-white border-red-600' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'" @click="emit('toggleExtracted', id)">Ext</button>
                </div>
              </div>
              <div v-if="gIdx !== UPPER_ARCH.length - 1" class="w-4"></div>
            </template>
        ...
        <div class="flex flex-col gap-10 mb-6">
          <ToothImageRow label="BUCCAL" :arch="UPPER_ARCH" :chart-data="chartData" surface="buccal" :selected-tooth-id="selectedToothId" grid-class="clinical-grid-bg" image-class="scale-x-[-1]" label-position="top" />
          <ToothImageRow label="PALATAL" :arch="UPPER_ARCH" :chart-data="chartData" surface="lingual" :selected-tooth-id="selectedToothId" grid-class="clinical-grid-bg" image-class="scale-x-[-1]" label-position="top" />
        </div>

        <!-- Upper Arch Palatal -->
        <div class="flex mt-6 mb-16">
          <div class="flex flex-col bg-white border-l border-y border-slate-300 text-[9px] font-bold text-slate-500 w-20 sticky left-0 z-20">
            <div v-for="row in PALATAL_ROWS" :key="row" class="h-6 flex items-center px-2 border-b border-r border-slate-200 last:border-b-0">{{ row }}</div>
          </div>
          <div class="flex">
            <template v-for="(group, gIdx) in UPPER_ARCH" :key="gIdx">
        ...
        <div class="flex items-end mb-1 mt-12">
          ...
          <div class="flex">
            <template v-for="(group, gIdx) in LOWER_ARCH" :key="gIdx">
              ...
              <div v-if="gIdx !== LOWER_ARCH.length - 1" class="w-4"></div>
            </template>
          </div>
        </div>
        ...
        <div class="flex mt-1 mb-8">
          ...
          <div class="flex">
            <template v-for="(group, gIdx) in LOWER_ARCH" :key="gIdx">
              <div class="flex">
                <div v-for="id in group" :key="id" class="w-12 sm:w-[54px] h-8 flex items-center justify-center">
                  <button class="w-full h-6 text-[9px] font-black uppercase transition-all duration-200 border border-slate-200" :class="chartData[id].extracted ? 'bg-red-500 text-white border-red-600' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'" @click="emit('toggleExtracted', id)">Ext</button>
                </div>
              </div>
              <div v-if="gIdx !== LOWER_ARCH.length - 1" class="w-4"></div>
            </template>
        ...
        <div class="flex flex-col gap-10 mb-6">
          <ToothImageRow label="LINGUAL" :arch="LOWER_ARCH" :chart-data="chartData" surface="lingual" :selected-tooth-id="selectedToothId" grid-class="clinical-grid-bg-inf" image-class="scale-y-[-1] scale-x-[-1]" label-position="bottom" />
          <ToothImageRow label="BUCCAL" :arch="LOWER_ARCH" :chart-data="chartData" surface="buccal" :selected-tooth-id="selectedToothId" grid-class="clinical-grid-bg-inf" image-class="scale-y-[-1] scale-x-[-1]" label-position="bottom" />
        </div>
